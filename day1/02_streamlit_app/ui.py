...
def display_chat_page(pipe):
    ...
    if st.session_state.current_question and st.session_state.current_answer:
        ...
        st.info(f"応答時間: {st.session_state.response_time/60:.2f}分")
...
def display_history_list(history_df):
    ...
    filtered_df = history_df if filter_value is None else history_df[history_df["is_correct"].notna() & (history_df["is_correct"] == filter_value)]
    ...
    for _, row in paginated_df.iterrows():
        with st.expander(f"{row['timestamp']} - Q: {row['question'][:50] if row['question'] else 'N/A'}..."):
            ...
            cols[1].metric("応答時間(分)", f"{row['response_time']/60:.2f}")
...
def display_metrics_analysis(history_df):
    ...
    if valid_metric_options:
        ...
        if not chart_data.empty:
            chart_data = chart_data.assign(response_time_minutes=chart_data["response_time"] / 60)
            st.scatter_chart(chart_data, x="response_time_minutes", y=metric_option, color="正確性")
        else:
    ...
    if valid_stats_cols:
        metrics_stats = analysis_df[valid_stats_cols].assign(response_time_minutes=analysis_df["response_time"] / 60).describe()
    ...
    if valid_stats_cols and "正確性" in analysis_df.columns:
        try:
            accuracy_groups = analysis_df.groupby("正確性")[valid_stats_cols].mean().assign(response_time_minutes=lambda df: df["response_time"] / 60)
    ...
    if "response_time" in analysis_df.columns and analysis_df["response_time"].notna().any():
        ...
        if "id" in analysis_df.columns and analysis_df["id"].notna().any():
